...
parser.add_argument("--eval_dir", type=str, default=str(Path.home() /
                    "evaluation-pipeline-2024"), help="Path to the evaluation pipeline directory")
parser.add_argument("--skip_eval", action='store_true',
                    help="Do not run the evals")
...
eval_dataset = BabylmDataset(
    config['data']['eval_path'], SEQ_LENGTH, tokenizer=tokenizer, offset=0)
test_dataset = BabylmDataset(
    config['data']['test_path'], SEQ_LENGTH, tokenizer=tokenizer, offset=0)
...
if 'output_dir' in config['logging'] and 'output_path' not in config['logging']:
    ...
    raise (ValueError(
        'Please specify exactly one of "output_dir" or "output_path". For the latter, you can use the variables {NAME} and {ID}.'))
...
if __name__ == "__main__":
    ...
    wandb.init(project=config['logging']['project'],
               name=config['model']['name'], id=RUN_ID, config=config)
    ...
    eval_output = trainer.evaluation_loop(
        eval_dataloader, 'Best checkpoint', prediction_loss_only=True, metric_key_prefix='eval/best_ckpt')
    ...
    test_output = trainer.evaluation_loop(
        test_dataloader, 'Best checkpoint', prediction_loss_only=True, metric_key_prefix='test/best_ckpt')
    ...
    """
        ...
        if results_path.exists():
            ...
            wandb.log(
                {"blimp_averaged": (blimp_supplement_score + blimp_filtered_score)/2.0})
            print(
                f"Logged to wandb: blimp_supplement = {blimp_supplement_score}, blimp_filtered = {blimp_filtered_score}")
        else:
